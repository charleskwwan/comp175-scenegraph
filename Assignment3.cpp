...
int  main_window;
string filenamePath = "./data/custom.xml";
...
void setupCamera();
void flattenScene(SceneNode *root, vector<SceneObj> &objs);
...
void callback_load(int id) {
	...
	if (success == false) {
        ...
        glLoadIdentity();
        flattenScene(parser->getRootNode(), objs);
	}
}
...
void applyTransformation(SceneTransformation *trans) {
    Vector v;
    switch (trans->type) {
        ...
        case TRANSFORMATION_ROTATE:
            v = trans->rotate;
            glRotated(trans->angle, v[0], v[1], v[2]);
            break;
        case TRANSFORMATION_MATRIX:
            glMultMatrixd(trans->matrix.unpack());
            break;
    }
}
...
// renderScene
//      - renders a scene using a parse tree
//      - does not flatten
void renderScene(SceneNode *root, bool applyMat) {
    glPushMatrix();
    ...
    // render all primitives at this level
    size_t nprims = root->primitives.size();
    for (size_t i = 0; i < nprims; i++) {
        ScenePrimitive *prim = root->primitives[i];
        if (applyMat)
            applyMaterial(prim->material);
        renderShape(prim->type); // should draw
    }

    // recurse on children
    size_t nnodes = root->children.size();
    for (size_t i = 0; i < nnodes; i++)
        renderScene(root->children[i], applyMat);
...
void flattenScene(SceneNode *root, vector<SceneObj> &objs) {
    ...
    size_t nprims = root->primitives.size();
    for (size_t i = 0; i < nprims; i++) {
        SceneObj obj = { Matrix(transform), root->primitives[i] };
        objs.push_back(obj);
    }
    ...
    size_t nnodes = root->children.size();
    for (size_t i = 0; i < nnodes; i++)
        flattenScene(root->children[i], objs);

    glPopMatrix();
}


// renderFlattenedScene
//      - renders a flattened scene created flattenScene
void renderFlattenedScene(vector<SceneObj> &objs, bool applyMat) {
    for (vector<SceneObj>::iterator it = objs.begin(); it != objs.end(); it++) {
        glPushMatrix();

        glMultMatrixd(it->transform.unpack()); // mult not load to allow camera mods
        ScenePrimitive *prim = it->prim;
        if (applyMat)
            applyMaterial(prim->material);
        renderShape(prim->type); // should draw
...
void myGlutDisplay(void)
{
    ...
    Point eye(eyeX, eyeY, eyeZ); // resolve nonconst ref
    Vector look(lookX, lookY, lookZ); // resolve nonconst ref
    Vector up(0, 1, 0); // resolve nonconst ref
	...
	if (wireframe) {
        ...
        // renderScene(root, false);
        renderFlattenedScene(objs, false);
	}
	...
	if (fillObj == 1) {
        ...
        // renderScene(root, true);
        renderFlattenedScene(objs, true);
	}
	glDisable(GL_LIGHTING);
